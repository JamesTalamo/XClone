...
const Post = ({ post }) => {
	...
	const { mutate: deletePost, isPending: isDeleteing } = useMutation({
		mutationFn: async () => {
			try {
				const res = await fetch(`${import.meta.env.VITE_BACKEND_URI}/api/post/${post._id}`, {
					method: 'DELETE',
					headers: {
	...
	const { mutate: likePost, isPending: isLiking } = useMutation({
		mutationFn: async () => {
			try {
				const res = await fetch(`${import.meta.env.VITE_BACKEND_URI}/api/post/like/${post._id}`, {
					method: 'POST',
					headers: {
						'content-type': 'application/json'
					},
	...
	const { mutate: commentPost, isPending: isCommenting } = useMutation({
		mutationFn: async () => {
			try {
				const res = await fetch(`${import.meta.env.VITE_BACKEND_URI}/api/post/comment/${post._id}`, {
					method: 'POST',
					headers: {
						'content-type': 'application/json'
					},
					...
					<div className='flex justify-between mt-3'>
						<div className='flex gap-4 items-center w-2/3 justify-between'>
							...
							<dialog id={`comments_modal${post._id}`} className='modal border-none outline-none'>
								<div className='modal-box rounded border border-gray-600 bg-gray-900'>
									...
									>
										<textarea
											className='textarea w-full p-1 rounded text-md resize-none border focus:outline-none  border-gray-800 bg-gray-800 text-white'
